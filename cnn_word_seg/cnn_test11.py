...
# def cnn_train( pure_txts , pure_tags, word_id , tag2vec , config,vocabulary_size, epoch=300 ,):
#     embedding_size = 128
#     keep_prob = tf.placeholder(tf.float32)
#
#     embeddings = tf.Variable(tf.random_uniform([vocabulary_size, embedding_size], -1.0, 1.0), dtype=tf.float32)
...
#
#     for i in range(epoch):
#         temp_data = tqdm.tqdm(data(pure_txts , pure_tags, word_id , tag2vec , batch_size= 512 ),desc=u'Epoch %s Accuracy:0.0'%(i+1))
#         k = 0
#         accs = []
#         for x_data ,y_data in temp_data:
#             k += 1
#             if k%100 ==0:
#                 acc = sess.run(accuracy,feed_dict={x:x_data,y_:y_data,keep_prob:1.0})
#                 accs.append(acc)
...
#
#     saver = tf.train.Saver()
#     saver.save(sess , './model_data/first_model.ckpt')
...
#     if flat:
#         json.dump(word_id,open('vacubulary.json','w'))
...
if __name__ == '__main__':
    ...
    stops =u'，。！？；、：,\.!\?;:\n'
    txt=None
    with open('corpus_data/msr_training.utf8' ,encoding='UTF-8') as f:
        txt=f.read()
    ...
    word_count,word_id,vocabulary_size=word2dic(pure_txts,flat=True)

    tag2vec = {'S': [1, 0, 0, 0], 'B': [0, 1, 0, 0], 'M': [0, 0, 1, 0], 'E': [0, 0, 0, 1]}

   # config=make_default()
    #cnn_train(pure_txts,pure_tags,word_id,tag2vec,config,epoch=200)
